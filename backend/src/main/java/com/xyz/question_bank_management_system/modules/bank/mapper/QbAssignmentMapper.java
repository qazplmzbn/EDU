...
public interface QbAssignmentMapper {
    ...
    @Select({
            ...
            "  AND (",
            "    EXISTS (SELECT 1 FROM qb_assignment_target t1",
            "            WHERE t1.assignment_id=a.id AND t1.target_type='student' AND t1.student_id=#{userId})",
            "    OR EXISTS (",
            "      SELECT 1 FROM qb_assignment_target t2",
            "      JOIN qb_class_member cm1 ON cm1.class_id=t2.class_id",
            "      WHERE t2.assignment_id=a.id AND t2.target_type='class' AND cm1.student_id=#{userId}",
            "    )",
            "  )",
